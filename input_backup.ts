...
class EnvVarMissingError extends Error {
  constructor(public varName: string) {
    super(`Environment variable required: ${varName}`);
...
export function loadPrivateKeySync(): string | undefined {
...
const MAX_ITERATIONS = 200000; // enforce a reasonable upper limit — tune for your environment
...
export async function verifyPassword(password: string, storedHash: string): Promise<boolean> {
  ...
  try {
    ...
    if (iterations > MAX_ITERATIONS) return false; // mitigate attacker-supplied expensive iterations
