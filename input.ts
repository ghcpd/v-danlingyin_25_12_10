...
function getEnv(name: string): string {
  ...
  if (!v) {
    // avoid embedding secret/env names in thrown messages to reduce information leakage
...
// Protect PBKDF2 usage from overly-high iteration counts which could be abused for DoS
...
export function verifyPassword(password: string, storedHash: string): boolean {
  ...
  try {
    ...
    if (iterations > MAX_PBKDF2_ITERATIONS) {
      // iterations too large — treat as invalid/unknown hash to avoid expensive work
      return false;
    }
